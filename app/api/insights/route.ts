...
export async function GET(req: NextRequest) {
  ...
  if (pageErr)
    return NextResponse.json(
      { error: `allpage read failed: ${pageErr.message}` },
      { status: 500 },
    );

  const accountIds = (pageRows ?? [])
    .map((r: { account_id: string }) => r.account_id)
    .filter(Boolean);
  if (!accountIds.length)
    return NextResponse.json(
      { error: "allpage table is empty — run Get All Page first" },
      { status: 400 },
    );
  ...
  if (wantSummary) {
    ...
    for (const accountId of accountIds) {
      ...
      totalReach += items.reduce(
        (s, item) => s + parseInt(String(item.reach ?? "0"), 10),
        0,
      );
    }
